...
import { Pencil, PencilOff, MousePointer2, RotateCcw, Scissors, Copy, Clipboard, Trash2, FilePlus2, Import, MapPin, Waves, Building, Route, LucideIcon, Upload, Map, Mountain, Trees, LandPlot, Warehouse, Paintbrush, Save, PaintbrushVertical } from 'lucide-react';
import { LayerItem, FeatureToolbarProps, ToolItem, LayerNode, LayerGroup } from '../types/types';
...
const FeatureToolbar: React.FC<FeatureToolbarProps> = ({ setLayers, selectedLayerId }) => {
    ...
    const getIconComponent = (iconValue: string): React.ReactNode => {
        ...
        return selectedIcon ? <selectedIcon.Icon className="h-4 w-4" /> : <MapPin className="h-4 w-4" />;
    };
    ...
    const handleCreateNewFeature = () => {
        ...
        if (!newId.trim() || !newName.trim() || !newType.trim() || !newIcon.trim() || !newSymbology.trim()) {
            setCreateNewFeatureError(language === 'zh' ? '所有字段不能为空' : 'All fields are required');
            return;
        }
    ...
    const handleDelete = () => console.log(language === 'zh' ? '点击了删除要素' : 'Delete tool clicked');
    ...
    return (
        <>
            <Dialog open={createNewFeatureDialog} onOpenChange={setCreateNewFeatureDialog}>
                ...
                >
                    <DialogHeader>
                        <DialogTitle>{language === 'zh' ? '创建新要素' : 'Create New Feature'}</DialogTitle>
                        <DialogDescription>
                            {language === 'zh'
                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        <div className="flex items-center gap-6">
                            ...
                            >
                                <SelectTrigger className="flex-1 w-full cursor-pointer">
                                    <SelectValue placeholder={language === 'zh' ? '选择要素类型' : 'Select Feature Type'} />
                                </SelectTrigger>
                                <SelectContent>
                                    <SelectGroup>
                                        <SelectItem className='cursor-pointer hover:bg-gray-200' value="river">{language === 'zh' ? '河流' : 'River'}</SelectItem>
                                        <SelectItem className='cursor-pointer hover:bg-gray-200' value="building">{language === 'zh' ? '建筑' : 'Building'}</SelectItem>
                                        <SelectItem className='cursor-pointer hover:bg-gray-200' value="road">{language === 'zh' ? '道路' : 'Road'}</SelectItem>
                                    </SelectGroup>
                                </SelectContent>
                            </Select>
                        </div>
                        <div className="flex items-center gap-6">
                            ...
                            >
                                <SelectTrigger className="flex-1 w-full cursor-pointer">
                                    <SelectValue placeholder={language === 'zh' ? '选择图标' : 'Select Icon'} />
                                </SelectTrigger>
                                <SelectContent>
                                    <SelectGroup>
                                        {iconOptions.map(option => (
                                            <SelectItem key={option.value} className='cursor-pointer hover:bg-gray-200' value={option.value}>
                        ...
                        <div className="flex items-center gap-5">
                            ...
                            <Select
                                value={newSymbology}
                                onValueChange={setNewSymbology}
                            >
                                <SelectTrigger className="flex-1 w-full cursor-pointer">
                                    <SelectValue placeholder={language === 'zh' ? '选择符号样式' : 'Select Symbology'} />
                                </SelectTrigger>
                                <SelectContent>
                                    <SelectGroup>
            ...
            <Dialog open={importFeatureDialog} onOpenChange={setImportFeatureDialog}>
                ...
                >
                    <DialogHeader>
                        <DialogTitle className='text-2xl'>{language === 'zh' ? '导入要素' : 'Import Feature'}</DialogTitle>
                        <DialogDescription>
                            {language === 'zh'
                                ? '请选择数据来源并上传要素数据'
                            ...
                            >
                                ...
                                <div className="flex items-center space-x-2">
                                    <RadioGroupItem
                                        ...
                                        className='cursor-pointer'
                                    />
                                    <Label>{language === 'zh' ? '已注册数据' : 'Registered Data'}</Label>
                                </div>
                            </RadioGroup>
                        </div>
                        <div className='border-2 rounded-md p-3 space-y-2'>
                    ...
                    {editSwitch.map((tool, index) => (
                        <button
                            ...
                            className={cn(
                                ...
                                isEditSwitchAllowed ? "hover:bg-gray-200 cursor-pointer" : "cursor-not-allowed"
                            )}
                            title={tool.title}
                            disabled={!isEditSwitchAllowed}
                        >
                            <tool.Icon className={cn("w-6 h-6", isEditSwitchAllowed ? "text-gray-600" : "text-gray-400")} />
                        </button>
                    ))}
                </div>
                <Separator className='h-full ml-2 mr-2' orientation='vertical' />
