...
export default function InpsPage({ node }: InpsPageProps) {
	...
	const loadContext = async (node: SceneNode) => {
		pageContext.current = await InpsPageContext.create(node)
	}
	...
	const handleFileSelect = async () => {
		if (window.electronAPI && typeof window.electronAPI.openInpFileDialog === 'function') {
			try {
				...
				if (filePath) {
					if (pageContext.current) {
						pageContext.current.inpData.src_path = filePath
						triggerRepaint()
					}
				}
			} catch (error) {
	...
	const handleSaveInp = async () => {
		if (!pageContext.current?.inpData.name) {
			toast.warning('Please enter inp name')
			return
		}

		const gateData = {
			name: pageContext.current?.inpData.name,
			type: 'inp',
			src_path: pageContext.current?.inpData.src_path
		}
						...
						<div className='flex flex-col gap-4'>
							<div className='flex flex-col sm:flex-row gap-2 items-start sm:items-center'>
								...
								<Input
									...
									value={pageContext.current?.inpData.name}
									onChange={(e) => {
										pageContext.current!.inpData.name = e.target.value
										triggerRepaint()
									}}
							...
							<div className='flex flex-col sm:flex-row gap-2 items-start sm:items-center'>
								...
								<div className="flex flex-1 gap-2">
									<Input
										className='flex-1'
										value={pageContext.current?.inpData.src_path || ''}
										readOnly={true}
