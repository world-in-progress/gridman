...
export default function ResourceFolder({
    ...
    const localDropDownMenuComponent = (node: FileNode) => {
    ...
    const remoteDropDownMenuComponent = (node: FileNode) => {
        if (node.scenarioNodeName === "schemas") {
            return (
                <ContextMenuContent className="w-50 bg-white text-gray-900 border-gray-200">
                    <ContextMenuItem className="cursor-pointer" onClick={() => handleDropDownMenuOpen(node)}>
    ...
    // Local File Tree
    const renderLocalFileTree = (nodes: FileNode[], depth = 0): React.ReactNode => {
            ...
            <>
                <div key={node.path}>
                    ...
                    {node.type === "folder" &&
                        expandedFolders.has(node.name) &&
                        node.children &&
                        renderLocalFileTree(node.children, depth + 1)}
                </div>
            </>
        ))
    }
    ...
    // Remote File Tree
    const renderRemoteFileTree = (nodes: FileNode[], depth = 0): React.ReactNode => {
        return nodes.map((node) => (
            <>
                <div key={node.path}>
                    <ContextMenu>
                    ...
                    {node.type === "folder" &&
                        expandedFolders.has(node.name) &&
                        node.children &&
                        renderRemoteFileTree(node.children, depth + 1)}
                </div>
            </>
        ))
    }
    ...
    return (
        <div className="w-64 bg-gray-800 border-r border-gray-700">
            <div className="p-2">
                <div className="text-xs font-semibold text-gray-400 mb-2 uppercase tracking-wide">Explorer</div>
                <div className="max-h-full overflow-y-auto">
                    {renderLocalFileTree(localFileTree)}
                    {renderRemoteFileTree(remoteFileTree)}
                </div>
            </div>
        </div>
    )
